    ...
    <div className="space-y-6">
      ...
      {project.project_description && (
        <div className="rounded-xl bg-slate-800/50 p-4 border border-slate-700">
          <h3 className="font-medium mb-2 text-white">Deskripsi</h3>
          <p className="text-sm text-slate-300">{project.project_description}</p>
        </div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-xl bg-slate-800/50 p-4 border border-slate-700">
          <h3 className="font-medium mb-2 text-white">Deadline</h3>
        ...
        <div className="rounded-xl bg-slate-800/50 p-4 border border-slate-700">
          <h3 className="font-medium mb-2 text-white">Tim yang Menangani</h3>
          <div className="flex flex-wrap gap-2">
            {project.project_teams.length === 0 ? (
              <p className="text-sm text-slate-400">Belum ada tim yang ditugaskan</p>
            ) : (
              project.project_teams.map((pt) => (
                <span 
                  key={pt.team_id} 
                  className="inline-flex items-center rounded-full bg-slate-700 text-slate-300 px-3 py-1 text-sm"
                >
